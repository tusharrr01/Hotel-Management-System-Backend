...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      // Auto-upgrade user to hotel_owner if they're trying to add a hotel
      ...
      // Auto-upgrade regular users to hotel_owner
      if (user.role === "user") {
        user.role = "hotel_owner";
        await user.save();
        console.log(`✅ User ${user.email} upgraded to hotel_owner role`);
      }

      // Check if user is hotel_owner or admin
